...
def get_path_of_hosts():
    if platform == "linux" or platform == "linux2" or platform == "MacOs" or platform == "darwin":
        return "/etc/hosts"
    elif platform == "win32":
...
def unblock(sites):
    for name in sites:
        ...
        with open(get_path_of_hosts(), 'w') as file:
            file.write(new_text)
        print("{} is unblocked".format(name) if flag is True else "the site {} wasn't block".format(name))
...
# block_sites(["https://yownload.ml/","http://www.underwar.co.il/5-CS/#cat501"])
# unblock(["http://www.underwar.co.il/5-CS/#cat501"])